...
const CreateTeam = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    const [filter, setFilter] = useState([ 
        ...
        { name: "Credits", field: "rating", ascending: false, id: 3}
      ]);
    ...
    const handleCardClick = (data) => {
      const { pid, tid, role = playing_role || role } = data;
      if(selectedPlayerIds.length >= data?.gameCategoryData?.MaxPlayer) {
        console.log("Max Plyers selected")
        return
      }else{
        console.log("first")
      }
      ...
      if (index === -1) {
        setCredit(credit - (data?.fantasy_player_rating || data?.rating || data?.fantasy_credit));
        ...
        setCredit(credit + Number(data?.fantasy_player_rating || data?.rating || data?.fantasy_credit));
    ...
    useEffect(() => {
      let dataExtract;
      if(state?.game === 'Cricket')  dataExtract =  data?.playersList?.filter(player => player?.playing_role?.toLowerCase() === tabData[0]?.role?.toLowerCase());
      if(state?.game === 'Football')  dataExtract =  data?.playersList?.filter(player => player?.playing_role?.toLowerCase() === tabData[0]?.role?.toLowerCase());
      if(state?.game === 'Kabaddi')  dataExtract =  data?.playersList?.filter(player => player?.role?.toLowerCase() === tabData[0]?.role?.toLowerCase());
      setFilterData(dataExtract)
      setCredit(data?.gameCategoryData?.TotalCredit)
      ...
      if (data) {
        const initialRoleCount = data?.playersList?.reduce((acc, player) => {
          const { role } = player;
          acc[role] = 0;
          return acc;
        }, {});
        setRoleCount(initialRoleCount);
      }
      setTabData(
        state?.game === "Cricket"
          ? [
              { name: "WK", role: "wk", value: 0, span: roleCount?.wk },
              { name: "BAT", role: "bat", value: 1, span: roleCount?.bat },
              { name: "AR", role: "all", value: 2, span: roleCount?.all },
              { name: "BOWL", role: "bowl", value: 3, span: roleCount?.bowl },
            ]
          : state?.game === "Football"
          ? [
              { name: "GK", role: "G", value: 0, span: roleCount?.G },
              { name: "DEF", role: "D", value: 1, span: roleCount?.defender },
              { name: "MID", role: "M", value: 2, span: roleCount?.midfielder },
              { name: "ST", role: "F", value: 3, span: roleCount?.forward },
            ]
          : [
              {
                name: "DEF",
                role: "defender",
                value: 0,
                span: roleCount?.defender || 0,
              },
              {
                name: "ALL",
                role: "allrounder",
                value: 1,
                span: roleCount?.allrounder || 0,
              },
              {
                name: "Raiders",
                role: "raider",
                value: 2,
                span: roleCount?.raider || 0,
              },
            ]
      );
